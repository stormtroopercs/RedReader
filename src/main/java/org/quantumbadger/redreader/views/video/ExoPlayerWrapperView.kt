...
class ExoPlayerWrapperView(
    ...
    init {
        ...
        if (PrefsUtility.pref_behaviour_video_playback_controls()) {
            ...
            if (PrefsUtility.pref_behaviour_video_frame_step()) {
                val frameDuration = (1000f / (if (mVideoPlayer.getVideoFormat() != null)
    ...
    fun updateProgress() {
        if (mTimeBarView != null && mTimeTextView != null && !mReleased) {
            ...
            if (durationMs > 0) {
                ...
                val newText: String = (msToMinutesAndSecondsString(currentPositionMs)
                        + " / "
    ...
    private fun openSpeedSettingDialog(context: Context) {
        ...
        builder.setOnCancelListener(DialogInterface.OnCancelListener { dialog: DialogInterface? ->
    ...
    private fun addPresetSpeedButton(
        rowLayout: LinearLayout, label: String?,
        progress: Int, seekBar: SeekBar,
        speedValue: TextView, context: Context?
    ) {
        ...
        speedButton.setOnClickListener(OnClickListener { v: View? ->
