...
class JsonObject(parser: JsonParser) :
	JsonValue(),
	...
	init {
		...
		while ((parser.currentToken().also { jt = it }) != JsonToken.END_OBJECT) {
			if (jt != JsonToken.FIELD_NAME) {
				throw JsonParseException(
					parser,
					"Expecting field name, got " + jt.name,
					parser.currentLocation(),
				)
			}
	...
	public override fun asObject(): JsonObject = this
	...
	override fun iterator(): MutableIterator<MutableMap.MutableEntry<String, JsonValue>> = properties.entries.iterator()
