...
class WeakCache<K, V : WritableObject<K>, F>(private val cacheDataSource: CacheDataSource<K, V, F>) : CacheDataSource<K, V, F> {
	...
	override fun performRequest(
		...
		if (!keysRemaining.isEmpty()) {
			val oldestTimestampOuter = oldestTimestamp
