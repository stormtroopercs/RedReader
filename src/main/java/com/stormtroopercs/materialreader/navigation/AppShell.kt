 ...
 * navigation bar (Posts / Explore). Shown only when the user is at the root of
 * a top-level tab, so pushed screens (a post list, comments, …) go full-bleed
 * with their own app bar. The left navigation drawer (the `BOTH`/`DRAWER`
 * modes of the `navigation_type` pref) is added in a later increment; the
 * default bottom-bar mode is what ships here.
 */
@Composable
fun AppShell(
	...
	val showBottomBar = isPostsRoot || isExploreRoot
	...
	Scaffold(
		bottomBar = {
			if (showBottomBar) {
				NavigationBar {
					NavigationBarItem(
						selected = isPostsRoot,
						onClick = { navigationState.switchTopLevel(Main) },
						icon = {
							Icon(
					...
					NavigationBarItem(
						...
						onClick = { navigationState.switchTopLevel(Explore) },
						icon = {
							Icon(
