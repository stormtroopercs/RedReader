 ...
 ******************************************************************************/
package com.stormtroopercs.materialreader.test.markdown

import com.stormtroopercs.materialreader.reddit.prepared.markdown.CharArrSubstring
import com.stormtroopercs.materialreader.reddit.prepared.markdown.IntArrayLengthPair
import com.stormtroopercs.materialreader.reddit.prepared.markdown.MarkdownTokenizer
...
class MarkdownTokenizerTest {
	...
	private companion object {
		...
		private fun toCAS(s: String): CharArrSubstring = CharArrSubstring.generate(s.toCharArray())
	...
	fun testTokenizeItalic1() {
		...
		val expected = intArrayOf(
			'a'.code,
			' '.code,
			MarkdownTokenizer.TOKEN_ASTERISK,
			'b'.code,
			MarkdownTokenizer.TOKEN_ASTERISK,
		)
	...
	fun testTokenizeBold1() {
		...
		val expected = intArrayOf(
			'a'.code,
			' '.code,
			MarkdownTokenizer.TOKEN_ASTERISK_DOUBLE,
			'b'.code,
			MarkdownTokenizer.TOKEN_ASTERISK_DOUBLE,
		)
	...
	fun testTokenizeUnderscore2() {
		...
		val expected = intArrayOf(
			MarkdownTokenizer.TOKEN_UNDERSCORE,
			'a'.code,
			'b'.code,
			'c'.code,
			'd'.code,
	...
	fun testFindCloseWellBracketed1() {
		assertEquals(
			MarkdownTokenizer.findCloseWellBracketed(
